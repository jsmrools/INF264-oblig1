...
def gini_index(y: np.ndarray) -> float:    #Get the probabilities of each class in y 
...
def entropy(y: np.ndarray) -> float:    #Different approach to get the probabilities of each class in y 
    probabilities = count(y)
    ...
    for probability in probabilities:
        if probability >0: 
            ...
            multiplied_and_log_probalilities.append(0)  #we use a catch all to the cases where the probability is 0, because computing log(0) would cause an error
...
def most_common(y: np.ndarray) -> int: #returnes the most commen value
...
class Node:
    ...
    def is_leaf(self) -> bool: #used to check if node is leaf node
        return self.value is not None
    
    def to_string(self, depth=0, prefix=""): #a helper function to help us see the vishualized tree 
        indent = "    " * depth
        if self.is_leaf():
...
class DecisionTree:     #class that builds the decision tree
    ...
    def find_threshold(self, x_column: np.ndarray, y: np.ndarray): #Given a single feature column and labels y,find the threshold that gives the best information gain.
    ...
    def find_best_feature(self, X: np.ndarray, y: np.ndarray): #Loop over all features, call find_threshold for each,and return the best (feature, threshold).
    ...
    #2, choosing best split to recursively split the dataset so the labels 
    ...
    def fit(self, X: np.ndarray, y: np.ndarray,): #the recursive builder #
        self.root = self.build_tree(X, y, depth = 0)
    
    def traverse(self, node: Node, row: np.ndarray): #function that tells how to traverse the tree
        if node.is_leaf():
            return node.value
    ...
    def predict(self, X: np.ndarray) -> np.ndarray: #when given a NumPy array X of features, return a NumPy array of predicted integer labels.
...
if __name__ == "__main__":
    # Test the DecisionTree class on a dataset
